...
const autoBetPxNovigMultiplier = document.getElementById('auto-bet-px-novig-multiplier');
...
function showBetSuccessPopup(data) {
    ...
    const filterNameDisplay = filterName ? `<div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid #333;">
                ...
                <div style="color: #00ff88; font-size: 14px; font-weight: 500;">${escapeHtml(filterName)}</div>
            </div>` : '';
...
async function loadAutoBetSettings() {
    try {
        ...
        // Load ProphetX + Novig multiplier
        if (autoBetPxNovigMultiplier) {
            autoBetPxNovigMultiplier.value = data.px_novig_multiplier || 1.5;
        }
    ...
    try {
        ...
        const response = await fetch('/api/set_auto_bet', {
            ...
            body: JSON.stringify({
                ...
                px_novig_multiplier: autoBetPxNovigMultiplier ? parseFloat(autoBetPxNovigMultiplier.value) : 1.5,
...
if (autoBetPxNovigMultiplier) {
    autoBetPxNovigMultiplier.addEventListener('input', debouncedSave);
}
...
(function () {
    ...
    const tabOdds = document.getElementById('tab-btn-odds');
    const panelAlerts = document.getElementById('panel-alerts');
    const panelOdds = document.getElementById('panel-odds');
    if (!tabAlerts || !tabOdds || !panelAlerts || !panelOdds) {
        console.warn('[Kalshi UI] Tab controls missing — check dashboard.html for #tab-btn-dashboard / #tab-btn-odds');
        return;
    }

    const sportSel = document.getElementById('odds-sport-sel');
    const timingSel = document.getElementById('odds-timing-sel');
    const dateSel = document.getElementById('odds-date-sel');
    const autoRef = document.getElementById('odds-auto-refresh');
    const refBtn = document.getElementById('odds-refresh-btn');
    const meta = document.getElementById('odds-meta');
    const errBox = document.getElementById('odds-error');
    const thead = document.getElementById('odds-table-head');
    const tbody = document.getElementById('odds-table-body');

    let oddsTimer = null;

    function bumpOddsTimer() {
        if (oddsTimer) clearInterval(oddsTimer);
        oddsTimer = null;
        if (autoRef && autoRef.checked && !panelOdds.hidden) {
            oddsTimer = setInterval(loadOddsOnce, 6000);
        }
    }

    function setTab(name) {
        const onAlerts = name === 'alerts';
        tabAlerts.classList.toggle('active', onAlerts);
        tabOdds.classList.toggle('active', !onAlerts);
        tabAlerts.setAttribute('aria-selected', onAlerts ? 'true' : 'false');
        tabOdds.setAttribute('aria-selected', onAlerts ? 'false' : 'true');
        panelAlerts.hidden = !onAlerts;
        panelOdds.hidden = onAlerts;
        document.body.classList.toggle('view-odds', !onAlerts);
        const url = new URL(window.location.href);
        if (onAlerts) url.searchParams.delete('tab');
        else url.searchParams.set('tab', 'odds');
    ...
    tabOdds.addEventListener('click', () => {
        setTab('odds');
        loadOddsOnce();
    });

    const params = new URLSearchParams(window.location.search);
    if (params.get('tab') === 'odds') {
        setTab('odds');
        loadOddsOnce();
    }
    ...
    function renderOddsTable(data) {
        ...
        h += '<th>Start</th><th>Lv</th>';
        books.forEach((b) => {
            h += `<th title="${escapeHtml(b)}">${escapeHtml(abbrevBook(b))}</th>`;
        });
        h += '<th>Best</th>';
        hdr.innerHTML = h;
        thead.innerHTML = '';
        ...
        evs.forEach((row) => {
            ...
            books.forEach((b) => {
                const p = pr[b] || {};
                const am = p.away_am;
                const isB = b === best.away_book && am != null;
                cA += `<td class="pto-odds-cell ${am == null ? 'pto-muted' : ''} ${isB ? 'pto-best' : ''}">${fmtAm(am)}</td>`;
            });
            cA += `<td class="pto-odds-cell pto-best">${escapeHtml((best.away_book || '').slice(0, 8))} ${fmtAm(best.away_am)}</td>`;
            trA.innerHTML = cA;
            tbody.appendChild(trA);

            const trH = document.createElement('tr');
            let cH = `<td class="pto-side-label">${escapeHtml(home)}</td>`;
            books.forEach((b) => {
                const p = pr[b] || {};
                const am = p.home_am;
                const isB = b === best.home_book && am != null;
                cH += `<td class="pto-odds-cell ${am == null ? 'pto-muted' : ''} ${isB ? 'pto-best' : ''}">${fmtAm(am)}</td>`;
            });
            cH += `<td class="pto-odds-cell pto-best">${escapeHtml((best.home_book || '').slice(0, 8))} ${fmtAm(best.home_am)}</td>`;
            trH.innerHTML = cH;
    ...
    if (refBtn) refBtn.addEventListener('click', loadOddsOnce);
